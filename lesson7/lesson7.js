...
function move() {
    ...
    // Определяем новую точку. 3е задание
    if (direction == 'x-') {
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x - 1))[0] || 
        document.getElementsByClassName('cell-' + (coord_y) + '-' + (FIELD_SIZE_X - 1))[0];
    }
    else if (direction == 'x+') {
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x + 1))[0] ||
        document.getElementsByClassName('cell-' + (coord_y) + '-' + 0)[0];
    }
    else if (direction == 'y+') {
        new_unit = document.getElementsByClassName('cell-' + (coord_y - 1) + '-' + (coord_x))[0] ||
        document.getElementsByClassName('cell-' + (FIELD_SIZE_Y - 1) + '-' + (coord_x))[0];
    }
    else if (direction == 'y-') {
        new_unit = document.getElementsByClassName('cell-' + (coord_y + 1) + '-' + (coord_x))[0] ||
        document.getElementsByClassName('cell-' + 0 + '-' + (coord_x))[0];
    }
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        // 1 задание
        document.querySelector('.score').innerHTML = `Набранные очки: ${score}`;
    }
    return check;
}
